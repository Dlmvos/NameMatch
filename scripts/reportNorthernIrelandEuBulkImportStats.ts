/**
 * Read-only report: Northern Ireland EU rows from the Northern Ireland Statistics and Research Agency (NISRA) bulk JSONL pipeline.
 * Matches `meaning_source = 'Northern Ireland national statistics (eu-ni-nisra)'` (origin is null on import).
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:ni-eu-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

export const MEANING_SOURCE_BULK = 'Northern Ireland national statistics (eu-ni-nisra)';
...
async function main(): Promise<void> {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data: rows, error } = await supabase
      .from('baby_names')
      .select('id')
      .eq('country', 'Northern Ireland')
      .eq('region', 'EU')
      .or('is_premium.is.null,is_premium.eq.false')
      .eq('meaning_source', MEANING_SOURCE_BULK)
  ...
  const n = ids.length;
  console.log(
    `Northern Ireland EU bulk-import candidate rows (meaning_source = "${MEANING_SOURCE_BULK}"): ${n}`,
  );
