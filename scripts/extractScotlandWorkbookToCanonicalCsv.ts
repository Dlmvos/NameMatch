/**
 * Scotland recency-weighted workbook (.xlsx) → canonical CSV
 * (`year,name,sex,count,country`) for `adaptEuNationalCsvToJsonl.ts`.
 *
 * Scotland is published as a single national feed; this workbook is
 * the curated merge of NRS Scotland 1974–2024 baby-name file with exponential recency weighting (5-year half-life anchored at 2025) — see
 * `scripts/data/SCOTLAND_REAL_SOURCE_WORKFLOW.md`.
 *
...
export type ScotlandSheetMeta = { sheetName: string; sex: Sex };

export type ScotlandCanonicalRow = {
  year: number;
  name: string;
  sex: Sex;
  count: number;
...
const SHEET_TARGETS: ScotlandSheetMeta[] = [
...
function extractRowsFromSheet(
  rows: string[][],
  meta: ScotlandSheetMeta,
  country: string,
  emitYear: number,
  minWeightedCount: number,
): ScotlandCanonicalRow[] {
...
export function extractScotlandWorkbookToRecords(opts: {
  absInputPath: string;
  country?: string;
  emitYear?: number;
  minWeightedCount?: number;
}): ScotlandCanonicalRow[] {
  const country = opts.country ?? 'Scotland';
  ...
  const all: ScotlandCanonicalRow[] = [];
  ...
  for (const meta of SHEET_TARGETS) {
    ...
    console.log(
      `[sct-nrs] Sheet "${resolved}" (${meta.sex}, emit-year ${emitYear}): ${parsed.length} names`,
    );
    all.push(...parsed);
  }
...
function rowsToCanonicalCsv(rows: ScotlandCanonicalRow[]): string {
...
function main(): void {
  ...
  const records = extractScotlandWorkbookToRecords({
    absInputPath: absIn,
    country: cli.country,
  ...
  console.log(
    `[sct-nrs] Wrote ${records.length} canonical rows (boys ${boys}, girls ${girls}, emit-year ${cli.emitYear}) → ${cli.outputPath}`,
  );
  console.log(
    'Next: tsx scripts/adaptEuNationalCsvToJsonl.ts --in <this file> --out scripts/data/batches/external-eu-sct-nrs.v1.jsonl --slug eu-sct-nrs --top-per-year-sex 1200',
  );
}
