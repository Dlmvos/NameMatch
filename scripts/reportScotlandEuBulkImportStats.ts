/**
 * Read-only report: Scotland EU rows from the National Records of Scotland (NRS) bulk JSONL pipeline.
 * Matches `meaning_source = 'Scotland national statistics (eu-sct-nrs)'` (origin is null on import).
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:sct-eu-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

export const MEANING_SOURCE_BULK = 'Scotland national statistics (eu-sct-nrs)';
...
async function main(): Promise<void> {
  ...
  const n = ids.length;
  console.log(
    `Scotland EU bulk-import candidate rows (meaning_source = "${MEANING_SOURCE_BULK}"): ${n}`,
  );
