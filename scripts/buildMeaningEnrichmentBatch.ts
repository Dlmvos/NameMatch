...
type EnrichmentUpdate = {
  ...
  meaning_language: 'en';
  meaning_gender_scope: 'any' | 'boy' | 'girl' | 'neutral';
...
function buildUpdates(
  ...
  for (const target of targets) {
    ...
    updates.push({
      ...
      meaning_origin: entry.origin,
      meaning_source: `${entry.source}:${entry.sourceDetail}`,
      meaning_confidence: confidence,
      meaning_verified: true,
      meaning_language: 'en',
      meaning_gender_scope: entry.gender === 'boy' || entry.gender === 'girl' || entry.gender === 'neutral'
        ? entry.gender
        : 'any',
...
function writeSqlUpdates(filePath: string, rows: EnrichmentUpdate[]): void {
  const lines = [
    ...
    '-- Safe to rerun: upserts canonical meanings and only fills empty baby_names meaning cache rows.',
    'begin;',
  ];
  const canonicalNames = new Map<string, string>();
  const canonicalMeaningKeys = new Set<string>();
  for (const row of rows) {
    if (!canonicalNames.has(row.normalized_name)) {
      canonicalNames.set(row.normalized_name, row.name);
    }
  }
  for (const [normalizedName, displayName] of canonicalNames) {
    lines.push(
      [
        'insert into public.canonical_names (normalized_key, display_name)',
        `values (${sqlLiteral(normalizedName)}, ${sqlLiteral(displayName)})`,
        'on conflict (normalized_key) do nothing;',
      ].join('\n'),
    );
  }
  for (const row of rows) {
    const canonicalMeaningKey = [
      row.normalized_name,
      row.meaning_language,
      row.meaning_gender_scope,
      row.meaning,
      row.meaning_source,
    ].join('\u0000');
    if (!canonicalMeaningKeys.has(canonicalMeaningKey)) {
      canonicalMeaningKeys.add(canonicalMeaningKey);
      lines.push(
        [
          ...
          'select',
          '  cn.id,',
          `  ${sqlLiteral(row.meaning)},`,
          `  ${sqlLiteral(row.meaning_origin)},`,
          `  ${sqlLiteral(row.meaning_gender_scope)},`,
          `  ${sqlLiteral(row.meaning_language)},`,
          `  ${sqlLiteral(row.meaning_source)},`,
          `  ${row.meaning_confidence},`,
          '  true',
          'from public.canonical_names cn',
          `where cn.normalized_key = ${sqlLiteral(row.normalized_name)}`,
    ...
    lines.push(
      [
        ...
        'from public.canonical_names cn',
        `where bn.id = ${sqlLiteral(row.id)}`,
        '  and bn.canonical_name_id is null',
        `  and cn.normalized_key = ${sqlLiteral(row.normalized_name)};`,
      ].join('\n'),
    );
    lines.push(
      [
