/**
 * Read-only report: England and Wales EU rows from the Office for National Statistics (ONS) bulk JSONL pipeline.
 * Matches `meaning_source = 'England and Wales national statistics (eu-ew-ons)'` (origin is null on import).
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:ew-eu-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

export const MEANING_SOURCE_BULK = 'England and Wales national statistics (eu-ew-ons)';
...
async function main(): Promise<void> {
  ...
  const n = ids.length;
  console.log(
    `England and Wales EU bulk-import candidate rows (meaning_source = "${MEANING_SOURCE_BULK}"): ${n}`,
  );
