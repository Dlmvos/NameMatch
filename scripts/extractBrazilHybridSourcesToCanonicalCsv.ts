/**
 * Brazil hybrid sources → canonical CSV (`year,name,sex,count,country`)
 * for `adaptEuNationalCsvToJsonl.ts` / `importBabyNamesFromJsonl.ts`.
 *
 * **Inputs:**
 * - **Top 150 workbook** (`Names Brasil top 150.xlsx`): expected to follow Portugal-style layout
 *   on sheet `Top names 5 year trend` — row 0–1 headers; from row 2:
 *   boys **Name** in column **C** (index 2), girls **Name** in column **H** (index 7).
 * - **Allowed-names PDF** (`brasil_allowed_names.pdf`): expected lines containing
 *   **`Femininos ...  Masculinos ...`** (two or more spaces between sides).
 *
 * **Merge / dedupe:** Key = normalized `name|sex`. **Top 150 wins** over PDF when both exist.
 *
 * **Pseudo-counts (documented, not real frequencies):**
 * one descending sequence where Top 150 ranks are always above PDF tail.
 *
 * Usage:
 *   tsx scripts/extractBrazilHybridSourcesToCanonicalCsv.ts \
 *     --top-xlsx "scripts/data/raw/downloads/Names Brasil top 150.xlsx" \
 *     --pdf scripts/data/raw/downloads/brasil_allowed_names.pdf \
 *     --out scripts/data/raw/latin-br-hybrid-2024.canonical.csv --year 2024
 */
...
function parseArgs(argv: string[]): Cli {
  ...
  const country =
    countryIdx >= 0 && argv[countryIdx + 1] ? String(argv[countryIdx + 1]).trim() : 'Brazil';
  return {
    topXlsx: argv[tx + 1],
    pdfPath: argv[pdfI + 1],
...
function parseAllowedNamesPdfText(text: string): { name: string; sex: Sex }[] {
  ...
  for (const rawLine of lines) {
    ...
    if (fName.length >= 1 && !/^femininos$/i.test(fName)) out.push({ name: fName, sex: 'F' });
    if (mName.length >= 1 && !/^masculinos$/i.test(mName)) out.push({ name: mName, sex: 'M' });
  }
  return out;
}
...
async function mainAsync(): Promise<void> {
  ...
  console.log(
    `[br-hybrid] Top 150 sheet "${sheetName}": ${topOrdered.length} names (deduped), PDF allowlist-only: ${pdfOnly.length}, total CSV rows: ${outLines.length - 1}`,
  );
  ...
  console.log(
    'Next: tsx scripts/adaptEuNationalCsvToJsonl.ts --in <this file> --out scripts/data/batches/external-latin-br-hybrid-2024.v1.jsonl --slug latin-br-ibge --output-region LATIN_AMERICA --top-per-year-sex 20000',
  );
}
