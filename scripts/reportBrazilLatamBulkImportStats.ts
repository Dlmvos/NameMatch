/**
 * Read-only report: Brazil LATIN_AMERICA rows produced by the bulk JSONL pipeline
 * (`adaptEuNationalCsvToJsonl`, national-statistics origin).
 * Use before scoped DELETE and after import for parity checks —
 * see `scripts/data/BRAZIL_REAL_SOURCE_WORKFLOW.md`.
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:br-latam-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

const ORIGIN_BULK_BR = 'Brazil (national statistics)';
...
async function main(): Promise<void> {
  ...
  const n = ids.length;
  console.log(
    `Brazil LATIN_AMERICA bulk-import candidate rows (origin = "${ORIGIN_BULK_BR}"): ${n}`,
  );
