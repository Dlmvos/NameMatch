/**
 * Read-only report: Netherlands EU rows produced by the bulk JSONL pipeline (`adaptEuNationalCsvToJsonl`, national-statistics origin).
 * Use before scoped DELETE + real NL workbook import — see `scripts/data/NETHERLANDS_REAL_SOURCE_WORKFLOW.md`.
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:nl-eu-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

const ORIGIN_BULK_NL = 'Netherlands (national statistics)';
...
async function main(): Promise<void> {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data: rows, error } = await supabase
      ...
      .eq('region', 'EU')
      // Match `netherlands_eu_bulk_delete_for_replacement.sql`: coalesce(is_premium, false) = false
      .or('is_premium.is.null,is_premium.eq.false')
      .eq('origin', ORIGIN_BULK_NL)
  ...
  const n = ids.length;
  console.log(`Netherlands EU bulk-import candidate rows (origin = "${ORIGIN_BULK_NL}"): ${n}`);
