/**
 * Read-only report: Australia WORLDWIDE rows from the RBDM-combined bulk JSONL pipeline.
 * Matches `meaning_source = 'Australia national statistics (au-rbdm-combined)'` (origin is null on import).
 *
 * Requires: EXPO_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
 *
 * Usage:
 *   npm run report:baby-names:au-worldwide-bulk-stats
 */
import 'dotenv/config';

import { createClient } from '@supabase/supabase-js';

export const MEANING_SOURCE_BULK_AU = 'Australia national statistics (au-rbdm-combined)';
...
async function main(): Promise<void> {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data: rows, error } = await supabase
      ...
      .eq('meaning_source', MEANING_SOURCE_BULK_AU)
  ...
  const n = ids.length;
  console.log(
    `Australia WORLDWIDE bulk-import candidate rows (meaning_source = "${MEANING_SOURCE_BULK_AU}"): ${n}`,
  );
